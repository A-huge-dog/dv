...
from infrastructure.persistence.artifact_store import load_index
from domain.repair import STAGE_READ_TOOLS
...
class ProjectReadModel:
    ...
    @staticmethod
    def _expected_artifact_roots(
            map1: Mapping[str, Any], map2: Mapping[str, Any],
            candidate: Mapping[str, Any]) -> dict[str, str]:
        return {
            "scenario_ac_map": str(map1["artifact_fingerprint"]),
            "ac_testcase_map": str(map2["artifact_fingerprint"]),
            "effective_uvm": str(candidate["effective_uvm_root"]),
            "testcase": str(candidate["candidate_fingerprint"]),
        }
    ...
    def from_checkpoint(
        ...
        expected_roots = cls._expected_artifact_roots(
            map1, map2, candidate)
