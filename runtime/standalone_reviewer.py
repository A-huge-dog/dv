...
from infrastructure.persistence.atomic_artifact import is_pending_artifact
...
from domain.review import (
    REVIEW_EVIDENCE_CHECK_TOOL, REVIEW_TOOL, build_review_report,
...
class StandaloneStage3ReviewerWorkflow(StandaloneStage3Workflow):
    ...
    def run(self, submission: dict[str, Any], submission_bytes: bytes | None = None) -> dict[str, Any]:
        ...
        effective_files, effective_root = load_effective_uvm_pass(
            ...
            input_fingerprint=source_value["input_fingerprint"],
            cycle_id="initial",
            stage2_root=map2["artifact_fingerprint"],
            error=ProjectJobError)
        if effective_root != candidate["effective_uvm_root"]:
            raise ProjectJobError(
                "STALE_EVIDENCE",
                "standalone Reviewer candidate UVM root is stale")
        uvm_context = self.staged._review_runtime_capability(
            source_value, effective_files, effective_root)
        request = build_review_request(execution, evidence, spec_fp, map1, map2, shards,
                                       candidate, reviewer, 1, ProjectJobError, routing,
                                       uvm_context=uvm_context)
        ...
        def preflight_review(arguments: dict[str, Any]) -> dict[str, Any]:
            return check_review_evidence(
                arguments, request, candidate, ProjectJobError)
