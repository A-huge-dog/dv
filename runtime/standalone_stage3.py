...
class StandaloneStage3Workflow:
    ...
    def run(
        ...
        effective_uvm_files, effective_uvm_root = load_effective_uvm_pass(
            source_root, job_id=value["job_id"],
            input_fingerprint=value["input_fingerprint"],
            cycle_id=cycle_id, stage2_root=map2["artifact_fingerprint"],
            error=ProjectJobError)
        candidate = self.staged.generate_stage3_handler.handle(
            GenerateStage3Input(
                ...
                effective_uvm_root=effective_uvm_root)).artifact
        store = self.staged._incremental_store(job_root)
        stage1_index, _ = store.load(
