...
from application.recompute_impact import (
    RecomputeImpactDependencies, RecomputeImpactHandler,
    RecomputeImpactInput,
)
from application.review import FinalReviewHandler, ReviewInput
from application.uvm_generation import (
    UvmGenerationInput, load_effective_uvm_pass,
)
from application.generation import GenerateStage3Input
from application.human_gate import CreateHumanGateInput
from domain.artifacts import (
    _artifact_paths, _artifact_roots, _rebuild_stage2, _rebuild_stage3,
    _validate_committed_candidate, validate_compile_validation,
)
...
from domain.agent_binding import binding, binding_lineage
from infrastructure.persistence.artifact_store import (
    IncrementalArtifactStore, load_index,
)
from runtime.errors import ProjectJobError
from runtime.project_job import (
    ProjectJobWorkflow, validate_project_input,
)
from application.scoped_repair import validate_scoped_replacement_lineage
from domain.artifacts import artifact_fingerprint
from domain.repair import (
    build_failure_feedback, canonical_repair_groups,
    dispatch_canonical_group,
)
...
from infrastructure.persistence.repair_records import (
    RepairRecordStore, build_prompt_contract,
)
...
def validate_commit_manifest(
    ...
    _validate_committed_candidate(candidate, ProjectJobError)
...
class ProjectCommitRuntime:
    ...
    def __init__(
        ...
        self.recompute_impact_handler = RecomputeImpactHandler(
            RecomputeImpactDependencies(
                error=ProjectJobError,
                record_store=RepairRecordStore,
    ...
    def _prepare_stage3(
        ...
        for candidate_path in sorted(job_root.glob(
            ...
            expected = _rebuild_stage3(
                ...
                int(existing["revision"]), ProjectJobError)
        ...
        if new_candidate is None:
            ...
            new_candidate = _rebuild_stage3(
                candidate, replacements, map1, map2, provider,
                revision, ProjectJobError)
        new_candidate["effective_uvm_root"] = effective_uvm_root
        new_candidate["upstream_fingerprints"]["effective_uvm"] = \
            effective_uvm_root
        new_candidate["validation"]["validation_fingerprint"] = canonical_hash({
            "content_fingerprint": new_candidate["content_fingerprint"],
            "input_fingerprint": new_candidate["input_fingerprint"],
            "scenario_ac_map_fingerprint":
                new_candidate["scenario_ac_map_fingerprint"],
            "ac_testcase_map_fingerprint":
                new_candidate["ac_testcase_map_fingerprint"],
            "effective_uvm_root": effective_uvm_root,
            "checks": new_candidate["validation"]["checks"],
        })
        new_candidate["candidate_fingerprint"] = artifact_fingerprint(
            new_candidate, "candidate_fingerprint")
        new_paths = {
            "scenario_ac_map": authority["scenario_ac_map_path"],
            "ac_testcase_map": authority["ac_testcase_map_path"],
    ...
    def _prepare_regenerated_stage3(
            self, job_root: Path, project_input: Mapping[str, Any],
            authority: Mapping[str, Any], staged: StagedProjectWorkflow,
            effective_uvm_files: tuple[dict[str, str], ...],
            effective_uvm_root: str) -> dict[str, Any]:
        """Run the fixed repair Stage 3 downstream after UVM has passed."""
        map1, map2, testcases, shards, prior = self._load_source_bundle(
            job_root, authority, staged)
        revision = max(int(prior["revision"]) + 1, int(map2["revision"]))
        workflow = staged.workflow
        workflow.role_providers["repair.stage3"] = self.provider_factory(
            job_root, project_input, "repair.stage3")
        workflow._probe_provider(job_root, "repair.stage3")
        spec_evidence, _, spec_fingerprint = staged._spec(dict(project_input))
        result = staged.generate_stage3_handler.handle(GenerateStage3Input(
            project_input=dict(project_input),
            job_root=job_root,
            spec_evidence=spec_evidence,
            spec_fingerprint=spec_fingerprint,
            scenario_ac_map=map1,
            ac_testcase_map=map2,
            testcases=testcases,
            shards=shards,
            revision=revision,
            budget=workflow._budget(),
            prior=prior,
            issues=[{
                "finding_id": "FRAMEWORK.STAGE2.DOWNSTREAM",
                "stage": "STAGE_3",
                "required_change": (
                    "Regenerate the complete thin testcase for the repaired "
                    "Stage 2 mapping and the current Xcelium-PASS UVM tree."),
            }],
            effective_uvm_files=effective_uvm_files,
            effective_uvm_root=effective_uvm_root,
        ...
        new_indexes = {
            **old_indexes,
            "stage3": _current_index_path("stage3", revision),
        }
        new_paths = _artifact_paths(map1, map2, candidate)
        return {
            "stage": "STAGE_3", "map1": map1, "map2": map2,
            "candidate": candidate,
            "old_artifact_paths": {
            ...
            "new_artifact_paths": new_paths,
            "old_artifact_roots": copy.deepcopy(authority["artifact_roots"]),
            "new_artifact_roots": _artifact_roots(map1, map2, candidate),
            "old_index_paths": old_indexes,
            "new_index_paths": new_indexes,
        }
    ...
    def _final_review_current_authority(
        ...
        uvm_files, uvm_root = load_effective_uvm_pass(
            job_root, job_id=project_input["job_id"],
            input_fingerprint=project_input["input_fingerprint"],
            cycle_id=("initial" if int(map2["revision"]) == 0 else
                      "repair-r{:03d}".format(int(map2["revision"]))),
            stage2_root=map2["artifact_fingerprint"], error=ProjectJobError)
        if candidate.get("effective_uvm_root") != uvm_root:
            raise ProjectJobError(
                "STALE_EVIDENCE", "Final Review UVM root is stale")
        review_handler = FinalReviewHandler(replace(
            review_workflow.final_review_handler.dependencies,
            uvm_context=review_workflow._review_runtime_capability(
                dict(project_input), uvm_files, uvm_root)))
        review_result = review_handler.handle(ReviewInput(
            "FINAL", dict(project_input), job_root, spec_evidence, sources,
            spec_fp, map1, map2, shards, candidate, reviewer_probe, 2,
        ...
        paths = review_result.output_references
        human_paths = {
    ...
    def _final_review_recovered(
        ...
        if candidate.get("effective_uvm_root") != uvm_root:
            raise ProjectJobError(
                "STALE_EVIDENCE", "recovered Final Review UVM root is stale")
        review_handler = FinalReviewHandler(replace(
            review_workflow.final_review_handler.dependencies,
            uvm_context=review_workflow._review_runtime_capability(
                dict(project_input), uvm_files, uvm_root)))
        ...
        human = review_workflow.create_human_gate_handler.handle(
            CreateHumanGateInput(
                ...
                review_storage_revision=storage_revision)).checkpoint
        dispatch = load_document(job_root / checkpoint["dispatch_path"])
        records = RepairRecordStore(
            job_root, job_id=project_input["job_id"],
            input_fingerprint=project_input["input_fingerprint"],
    ...
    def advance(self, job_id: str) -> dict[str, Any]:
        ...
        workflow = ProjectJobWorkflow(
            self.workspace_root, self.result_root,
            uvm_build_runner=self.uvm_build_runner)
        ...
        if pending_stage2:
            ...
            for dispatch, replacement, _ in entries:
                commit_result = self.commit_group_handler.handle(
                    CommitGroupInput(
                        job_root, project_input, dispatch, replacement,
                        prepared2))
                self.recompute_impact_handler.handle(RecomputeImpactInput(
                    job_root, project_input, dispatch, prepared2,
                    commit_result))
            authority = self._authority_checkpoint(
        ...
        cycle_id = (
            "initial" if int(map2["revision"]) == 0 else
            "repair-r{:03d}".format(int(map2["revision"])))
        try:
            effective_uvm_files, effective_uvm_root = load_effective_uvm_pass(
                job_root, job_id=project_input["job_id"],
                input_fingerprint=project_input["input_fingerprint"],
                cycle_id=cycle_id,
                stage2_root=map2["artifact_fingerprint"],
                error=ProjectJobError)
        except ProjectJobError as caught:
            if caught.code != "PARTIAL_ARTIFACT" or int(map2["revision"]) == 0:
                raise
            prior_map2 = load_document(
                job_root / initial_checkpoint["ac_testcase_map_path"])
            prior_cycle = (
                "initial" if int(prior_map2["revision"]) == 0 else
                "repair-r{:03d}".format(int(prior_map2["revision"])))
            baseline_files, baseline_root = load_effective_uvm_pass(
                job_root, job_id=project_input["job_id"],
                input_fingerprint=project_input["input_fingerprint"],
                cycle_id=prior_cycle,
                stage2_root=prior_map2["artifact_fingerprint"],
                error=ProjectJobError)
            workflow.role_providers["repair.uvm"] = self.provider_factory(
                job_root, project_input, "repair.uvm")
            spec_evidence, _, _ = staged._spec(dict(project_input))
            repair_testcases, repair_shards = staged._load_shards(
                job_root, map2)
            uvm_result = staged._run_uvm_worker(
                UvmGenerationInput(
                    project_input=dict(project_input), job_root=job_root,
                    spec_evidence=spec_evidence, stage2_artifact=map2,
                    logical_testcases=repair_testcases,
                    stage2_shards=repair_shards, cycle_id=cycle_id,
                    cycle_kind="repair", repair_revision=int(map2["revision"]),
                    baseline_files=baseline_files,
                    baseline_root=baseline_root))
            if uvm_result.state != "UVM_GENERATION_PASS":
                return {**copy.deepcopy(uvm_result.checkpoint),
                        "state": uvm_result.state}
            effective_uvm_files = uvm_result.effective_files
            effective_uvm_root = str(uvm_result.effective_uvm_root)
        ...
        stage3_current = (
            ...
            candidate.get("effective_uvm_root") == effective_uvm_root and
        ...
        if not stage3_current:
            ...
            for group in stage3_groups:
                ...
                dispatched.add(group["group_id"])
            try:
                prepared3 = self._prepare_regenerated_stage3(
                    job_root, project_input, authority, staged,
                    effective_uvm_files, effective_uvm_root)
            except ProjectJobError as caught:
                if caught.code not in {
                        "BLOCKED_TOOL", "ATTEMPT_PAUSED",
                        "TOOL_LIMIT_EXCEEDED"}:
                    raise
                return {
                    "state": "PAUSED_RETRYABLE",
                    ...
                    "artifact_roots": authority["artifact_roots"],
                    "diagnostic": {
                        "code": "STAGE3_PROVIDER_UNAVAILABLE",
                        "message": (
                            "repair Stage 3 Provider did not complete; rerun "
            ...
            for dispatch, replacement, _ in stage3_commits:
                commit_result = self.commit_group_handler.handle(
                    CommitGroupInput(
                        ...
                        prepared3))
                self.recompute_impact_handler.handle(RecomputeImpactInput(
                    job_root, project_input, dispatch, prepared3,
                    commit_result))
            authority = self._authority_checkpoint(
