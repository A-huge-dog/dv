...
from application.repair import (
    CreateRepairPlanDependencies, CreateRepairPlanHandler,
    CreateRepairPlanInput, ScopedReplacementDependencies,
    ScopedReplacementHandler, ScopedReplacementInput,
)
...
from domain.repair import validate_current_dispatch_authority
...
class ProjectRepairRuntime:
    ...
    def run_orchestrator(
        ...
        def submit(
            ...
            handler = CreateRepairPlanHandler(CreateRepairPlanDependencies(
                error=self.error, persist=self._persist, records=self._records))
            result = handler.handle(CreateRepairPlanInput(
                candidate=candidate, agent_context=context,
                project_input=self.project_input, checkpoint=self.checkpoint,
                report=report, review_request=request,
                artifact_roots=self.model.artifact_roots,
                inventory=self.inventory, read_model=self.model,
    ...
    def run_stage(
        ...
        def submit(
            ...
            handler = ScopedReplacementHandler(
                ScopedReplacementDependencies(
                    error=self.error,
                    validate_authority=self.validate_stage_authority,
